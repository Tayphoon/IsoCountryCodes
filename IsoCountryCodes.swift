...
public class IsoCountryCodes {
    ...
    public class func searchByName( name:String ) -> IsoCountryInfo {
    ...
    public class func searchByNumeric( numeric:String ) -> IsoCountryInfo {
        let country = IsoCountries.allCountries.filter( { $0.numeric == numeric } )

        return (!country.isEmpty) ? country[0] : IsoCountryInfo(name: "", numeric: "", alpha2: "", alpha3: "", calling: "", currency: "", continent: "")
    }

    public class func searchByCurrency( currency:String ) -> [IsoCountryInfo] {
        let country = IsoCountries.allCountries.filter(  { $0.currency == currency } )
        return country
    }

    public class func searchByCallingCode( calllingCode:String ) -> IsoCountryInfo {
